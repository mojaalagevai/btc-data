...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("crypto_data_fetch.log"), logging.StreamHandler()],
)
    ...
    def make_request(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"HTTP {response.status_code} error for proxied URL: {proxied_url}"
                )
                return {
                    "retCode": response.status_code,
                    "retMsg": f"HTTP {response.status_code} error",
                }
...
class CryptoDataFetcher:
    ...
    def __init__(
        self, coin: str = "BTC", base_currency: str = "USDT", testnet: bool = False
    ):
        ...
        self.categories = {"spot": "spot", "perpetual": "linear"}
    ...
    def get_previous_day_timestamps(self) -> Tuple[str, int, int]:
        ...
        start_time = int(
            yesterday_utc.replace(hour=0, minute=0, second=0, microsecond=0).timestamp()
            * 1000
        )
        end_time = int(
            yesterday_utc.replace(
    ...
    def test_server_time(self) -> bool:
        ...
        if response.get("retCode") == 0:
            ...
            logger.error(
                f"✗ Server time test failed: {response.get('retMsg', 'Unknown error')}"
            )
            return False

    def fetch_candle_data_via_proxy(
        self, category: str, symbol: str, interval: str, start_time: int, end_time: int
    ) -> Optional[Dict]:
    ...
    def fetch_candle_data_with_retry(
        self, category: str, interval: str, start_time: int, end_time: int
    ) -> Optional[Dict]:
        ...
        if candles_needed <= 1000:
            ...
            response = self._fetch_single_batch(
            ...
            batch_size_ms = (
                1000 * interval_minutes * 60 * 1000
            )  # 1000 candles worth of milliseconds

            while current_start < end_time:
                current_end = min(current_start + batch_size_ms, end_time)

                response = self._fetch_single_batch(
                    category, interval, current_start, current_end
                )
                if response and response.get("retCode") == 0:
            ...
            return {"retCode": 0, "retMsg": "OK", "result": {"list": all_candles}}
...
def _fetch_single_batch(
    ...
    for attempt in range(self.max_custom_retries):
        try:
            ...
            if response and response.get("retCode") == 0:
                ...
                error_code = (
                    response.get("retCode", "Unknown") if response else "No response"
                )
                error_msg = (
    ...
    def save_data_efficiently(
        ...
        try:
            ...
            if candles_data:
                ...
                logger.info(
                    f"Saved {category_name}/{folder_name} data: "
                    f"{len(candles_data)} candles for {self.symbol} to {dir_path}"
                )
                return True
            else:
                logger.warning(
                    f"No candle data to save for {category_name}/{folder_name} {self.symbol}"
                )
                return False

        except Exception as e:
            logger.error(
                f"Error saving {category_name}/{folder_name} data for {self.symbol}: {str(e)}"
            )
            return False
    ...
    def fetch_all_data(self) -> bool:
        ...
        logger.info(
            f"Data collection completed for {self.symbol}: {success_count}/{total_requests} successful"
        )
